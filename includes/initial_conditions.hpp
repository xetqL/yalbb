...
public:
    elements::ElementRealType min_r2;
    elements::ElementRealType T0;
...
class RandomElementsInClustersGenerator : public RandomElementsGenerator<N> {
    const int clusters, max_particles_per_cluster, seed, max_trial;
public:
    int number_of_clusters_generated = 0;

    RandomElementsInClustersGenerator(int clusters, const int max_particles_per_cluster, const int seed = __rd(), const int max_trial = 10000) :
            clusters(clusters),max_particles_per_cluster(max_particles_per_cluster), seed(seed), max_trial(max_trial) {}
    ...
    void generate_elements(std::vector<elements::Element<N>>& elements, const int n,
        ...
        int clusters_to_generate = clusters;
        int cluster_id = 0;
        ...
        while(cluster_id < clusters_to_generate && elements.size() < n) {
            ...
            statistic::NormalSphericalDistribution<N, elements::ElementRealType>
                    sphere_dist_position(condition->min_r2 * (max_particles_per_cluster), cluster_centerx, cluster_centery, cluster_centerz);
            statistic::NormalSphericalDistribution<N, elements::ElementRealType>
