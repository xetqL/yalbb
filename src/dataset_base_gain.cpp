...
int main(int argc, char** argv) {
    ...
    if(rank == 0){
        std::cout << "==============================================" << std::endl;
        std::cout << "= Computing times without load balancing ...  " << std::endl;
    ...
    if(rank == 0) std::cout << "Compute time within each hundred steps ... " << std::endl;
    compute_dataset_base_gain<DIMENSION>(fp, &mesh_data, zz, &params, MPI_COMM_WORLD);
